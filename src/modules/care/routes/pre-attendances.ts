...
export default async function preAttendanceRoutes(app: FastifyInstance) {
  ...
  app.get('/:id', {
    ...
    const item = await prisma.preAttendance.findUnique({ where: { id } });
  ...
  app.put('/:id', {
    ...
    try {
      const existing = await prisma.preAttendance.findUnique({ where: { id } });
      if (!existing) return reply.code(404).send({ error: 'Pre-attendance not found' });

      const item = await prisma.preAttendance.update({ where: { id }, data });
      return item;
    } catch (err: any) {
